export const sendTutorPermit = async (tutorId: string, userEmail: string ,price:number) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_API}/permits/create`, {
        method: "POST",
        headers: {
      ...
      return { success: true, message: data.message || "Permit sent successfully!" };
    } catch (error) {
  ...
  export const getAllPermits = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_API}/permits`,
        {
          next: {
  ...
  export const updatePermit = async (requestId: string, updateData: any) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_API}/permits/${requestId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
